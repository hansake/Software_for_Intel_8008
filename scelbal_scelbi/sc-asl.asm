	CPU 8008	
	RADIX 8
; This code was modified to assemble with the AS Assembler
; since the as8 assembler seems not awailable any more.
; The AS Assembler was modified to show octal addresses
; in listing if RADIX is set to 8. This is to simplify debugging.
;CONIO	EQU "SCELBI"
CONIO	EQU "THEI8008"
;;; This is the Scelbi Basic Program from 1974 known as
;;; SCELBAL.  This version is modified to assemble with the
;;; as8 assembler for the intel 8008 by Thomas E. Jones
...
;;; This code originates from a version made by Steve
;;; Loboco.
;;;
...
;ENDPGRAM  EQU 077 ;for ROMable implementation
;BGNPGRAM  EQU 044 ;       -"-
ENDPGRAM  EQU 055 ;MGA 4/10/12 as in orig
BGNPGRAM  EQU 033 ;MGA 4/10/12 as in orig
...
OLDPG1		EQU	(1*400)+000  ;001#000
OLDPG26		EQU	(26*400)+000 ;026#000
OLDPG27		EQU	(27*400)+000 ;027#000
OLDPG57		EQU	(57*400)+000 ;057#000


;;; Page zero will contain the I/O Routines.  These are actually
;;; just as suggested by Scelbal Manual for Serial I/O.
	JMP EXEC ;jump to EXEC also from beginning

	ORG 100		; save a bit of space before this
...
;    These functions support a simulated I/O for the
;        Intel 8008 Master Thesis computer (THEI8008).
;    Note that Inport 0 is read by the assembler code INP 0
;    Outport 0 is written by the assembler code OUT 10 (octal)
;
...
;    Outport 4: used to output character to tty printer.
;    Inport 5: used to input character from tty keyboard.
;
;    Inport 4: used for status flags for the ports (Flagport).
;      Flag 1 (bit 0): set to 1 when printer ready.
;      Flag 2 (bit 1): set to 1 when input available from tape-reader.
;      Flag 3 (bit 2): set to 1 when tape in tape-reader.
;      Flag 5 (bit 4): set to 1 when character available from keyboard.
...
CINP:   ; Get character from keyboard, return value of character in A register
	; the recieved character is echoed to the printer
	; for some reason in SCELBAL bit 7 is always "1" for the character
        INP 4           ;input value from status port
        NDI 20          ;test Flag 5 (bit 4) if character is available from keyboard
        JTZ CINP        ;no character available, test status again
        INP 5           ;get character
        OUT 14          ;print character
	ORI 200		;set bit 7 to "1"
        RET

CPRINT: ; Put charater to printer (or screen), output character in A register
	; register B is used to save the character
	; for some reason in SCELBAL bit 7 of the character is sometimes set
        LBA             ;save character to print i B register
STATST: INP 4           ;input value from status port
        NDI 1           ;test Flag 1 (bit 0) if printer is ready
        JTZ STATST      ;printer not ready, test again
        LAB
	CPI 224		;don't print CTRL-T
	RTZ
	NDI 177		;reset bit 7 of character
        OUT 14          ;print character
        RET
...
;;; Page one has many constants and variables.
	...
	DB 7 dup (?)	; NOT ASSIGNED (SHOULD BE 01 111-117)
	...
	DB 000,000,000,000
	DB 000,000,000,000
	DB 10 dup (?)		; 8 Bytes NOT ASSIGNED (SHOULD BE 01 170-01 177)
	DB 000,000,000,000	; TEMPORARY REGISTER STORAGE AREA (D,E,H&L)
	DB 4 dup (?)		; NOT ASSIGNED (01 204-01 207)
	...
	DB 6 dup (?)		; NOT ASSIGNED (01 221-01 226)
	...
	ORG (2*400)+000		; START PAGE 02, THE CODE
...
;;; NOW OPEN AREA UP TO 26 000 CAN BE USED FOR PATCHING...
	...
	ORG (26*400)+000
	DB 000			; CC FOR INPUT LINE BUFFER
	DB 117 dup (?) 		; 79 Bytes THE INPUT LINE BUFFER
	DB 000,000,000,000	; THESE ARE SYMBOL BUFFER STORAGE
	DB 000,000,000,000
	DB 000,000,000,000	; SHOULD BE 26-120 TO 26 143
	DB 000,000,000,000
	DB 000,000,000,000
	DB 000,000,000,000	; THESE LOCATIONS ARE AUXILIARY SYMBOL BUFFER
	DB 000,000,000,000
	DB 000,000,000,000	; SHOULD BE 26 144 TO 26 175
...
;;; NOW WE SHOULD BE UP TO 26 210
	...
	DB 17 dup (?)	; 15 Bytes OPERATOR STACK
	DB 000		; FUN/ARRAY STACK POINTER
	DB 7 dup (?)	; FUNCTION/ARRAY STACK
...
;;; This SHOULD START AT 26 300
	...
	DB 000,000,000,000	; (SHOULD BE 340-347)
	DB 000,000,000,000	; AUX LINE NUMBER BUFFER
	DB 000,000,000,000	; (SHOULD BE 350-357)
...
;;; locations 371-377 NOT ASSIGNED
	...
	DB 0			; END OF TABLE, SHOULD BE 072

	DB 000			; GOSUB STACK POINTER
	DB ?			; NOT ASSIGNED;
	DB 000			; NUMBER OF ARRAYS COUNTER
	DB 000			; ARRAY POINTER
	DB 000			; VARIABLES COUNTER SHOULD BE 077
	DB 000,000,000,000	; USED AS THE GOSUB STACK 100-117
	DB 000,000,000,000
	DB 000,000,000,000
	DB 000,000,000,000
	DB 000,000,000,000	; USED AS ARRAY VARIABLES TABLE
	DB 000,000,000,000	; SHOULD BE 120-137
	DB 000,000,000,000
	DB 000,000,000,000
	...
	DB 3 dup (?)	; NOT USED (SHOULD BE 205-207)
	DB 000		; USED AS VARIABLES SYMBOL TABLE
	DB 167 dup (?)	; 119 Bytes (SHOULD BE 211-377 RESERVED)
